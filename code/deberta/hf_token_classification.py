...
"""
# You can also adapt this script on your own token classification
# task and datasets. Pointers for this are left as comments.
...
def main(json_config_file_path: str = ""):
    ...
    model_args, data_args, training_args = parser.parse_json_file(
            json_file=os.path.abspath(json_config_file_path)
        )

    # if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
    #     # If we pass only one argument to the script and
    #     # it's the path to a json file,
    ...
    # model = DebertaV2ForTokenClassification(model_args,config=config)
    ...
    max_length = data_args.max_length if data_args.pad_to_max_length else 2048
    ...
    def tokenize_and_align_labels(examples):
        ...
        for i, label in enumerate(examples[label_column_name]):
            true_count=len(examples[text_column_name][i])
            ...
            # log_prob=examples["likelihood"][i]
            # rank=examples["rank"][i]
            # log_rank=examples["logrank"][i]
            # entropy=examples["entropy"][i]
            # log_prob = log_prob + [0] * (len(tokenized_inputs["input_ids"][i]) - len(examples["likelihood"][i]))
            # rank = rank + [0] * (len(tokenized_inputs["input_ids"][i]) - len(examples["rank"][i]))
            # log_rank = log_rank + [0] * (len(tokenized_inputs["input_ids"][i]) - len(examples["logrank"][i]))
            # entropy = entropy + [0] * (len(tokenized_inputs["input_ids"][i]) - len(examples["entropy"][i]))
        ...
        # tokenized_inputs["log_prob"]=log_probs
        # tokenized_inputs["rank"]=ranks 
        # tokenized_inputs["log_rank"]=log_ranks 
        # tokenized_inputs["entropy"]=entropys 
    ...
    if training_args.do_train:
        ...
        # train_dataset=train_dataset.remove_columns(['metrics'])
    if training_args.do_eval:
    ...
    def compute_metrics(p):
        ...
        accuracy = accuracy_score(merged_labels, merged_predictions)
        macro_f1 = f1_score(merged_labels, merged_predictions, average='macro')
        print("-- token-level metric --")
        print("Accuracy: {:.1f}".format(accuracy*100))
        print("Macro F1 Score: {:.1f}".format(macro_f1*100))

        precision = precision_score(merged_labels, merged_predictions, average=None)
        recall = recall_score(merged_labels, merged_predictions, average=None)
        print("Precision/Recall per class: ")
        precision_recall = ' '.join(["{:.1f}/{:.1f}".format(p*100, r*100) for p, r in zip(precision, recall)])
        print(precision_recall)

        return {"precision":precision.tolist(), "recall":recall.tolist(), "accuracy":accuracy, "macro_f1":macro_f1}
    ...
    if training_args.do_eval:
        ...
        output_val_json_predictions_file = os.path.join(
            training_args.output_dir, "val_predictions.json"
        )
        with open(output_val_json_predictions_file, "w") as f:
    ...
    if training_args.do_predict:
        ...
        true_predictions = [
            [int(label_list[p]) for (p, l) in zip(prediction, label) if l != -100]
        ...
        if trainer.is_world_process_zero():
            ...
            # save preds to json
            assert len(true_predictions) == len(test_dataset)

            data_list = []
            for i in range(len(true_predictions)):
                # if len(true_predictions[i]) < 512:
                #     true_predictions[i] = true_predictions[i] + true_predictions[i][-1] * (512 - len(true_predictions[i]))
                data_list.append(
                    {
